...
rfPred_scores_motor=function(variant_list, data, index, all.col, file.export, n.cores){
  ...
  # output columns
  names <- c("chromosome", "position_hg19", "reference", "alteration", "proteine",
             "aaref", "aaalt", "rfPred_score", "SIFT_score", "MutationTaster_score",
             "Polyphen2_score", "PhyloP_score", "LRT_score", "aapos", "Uniprot_id",
             "genename", "position_hg18", "Polyphen2_HDIV_score", "Polyphen2_HDIV_pred", 
             "Polyphen2_HVAR_score", "Polyphen2_HVAR_pred", "MutationTaster_pred", 
             "phyloP", "LRT_Omega", "LRT_pred")
  if (all.col){
    names.out <- names
  } else{
    names.out <- names[1:14]
  }

  # request on TabixFile (chr+pos)
  param <- reduce(GRanges(variant_list$chromosome,
                          IRanges(start=variant_list$position_hg19, width=1)),
  ...
  if (n.cores==1){
    ...
    res.rst <- unlist(parLapply(Clust, clusterSplit(Clust, param), 
                                function(variants, file){return(scanTabix(file, param=variants))},
                                TabixFile(data, index)))  
    stopCluster(Clust)
  }
  ...
  out <- merge(unique(variant_list[, 1:2, with=FALSE]), res.rst,
               all.x=TRUE, by=c("chromosome","position_hg19"),
  ...
  variant_list <- merge(variant_list, unique(res.rst[, 1:2, with=FALSE]),
                        by=c("chromosome","position_hg19"),
                        allow.cartesian=TRUE)
